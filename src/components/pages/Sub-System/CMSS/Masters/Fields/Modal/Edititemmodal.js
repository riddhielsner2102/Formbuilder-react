...
function Message({ setedit, seteditdata, editdata ,setdata}) {
  ...
  const formValues= { AppID: 13, FieldId: 0, IsActive:rowsdata[0].IsActive, FieldName: rowsdata[0].FieldName }
  ...
  const handleClose =async () => {
    const post=await PostRequest(URL, formValues);
    setedit(false);
    const response = await PrepareRequest(GetURL);
    setdata(response.data.lstMasterFields)
  }

  const handleChange = (e) => {
    let newdata = [...rowsdata]
    newdata[0].FieldName = e.target.value
